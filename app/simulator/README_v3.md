...
这是一个纯串口数据驱动的Python桌面程序，用于实时显示航模的飞行姿态和控制数据。
...
1. **纯数据驱动**: 不包含任何仿真逻辑，完全依赖串口数据
2. **实时处理**: 串口数据立即处理并显示，无延迟
3. **数据统计**: 实时计算数据更新频率和统计信息
4. **优化的渲染**: 使用更真实的颜色和材质效果
5. **多线程设计**: 串口读取在独立线程中运行
